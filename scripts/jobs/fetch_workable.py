...
def build_api_url(source_key: str, api_url: str) -> str | None:
    if api_url:
        return api_url
    if source_key:
        return f"https://www.workable.com/api/accounts/{source_key}?details=true"
    return None


def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch raw Workable jobs for verified sources.")
    ...
    sources = load_sources(provider="workable", statuses=args.status, source_ids=args.source_ids)
    if args.limit > 0:
    ...
    for source in sources:
        ...
        output_path = write_raw_snapshot("workable", source, api_url, payload)
        fetched += 1
        print(f"saved {source.company_name} -> {output_path}")

    print(f"workable complete: fetched={fetched} skipped={skipped}")
