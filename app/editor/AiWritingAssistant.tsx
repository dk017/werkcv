...
import { applyWritingChange, writingChanges, writingContextKey, type WritingChange, type WritingSelection } from "@/lib/ai-writing-changes";
...
export default function AiWritingAssistant(props: Props) {
  ...
  async function generate(regenerate?: WritingChange) {
    ...
    track("ai_writing_requested", { locale, target: analyticsTarget, action: selection.action, regeneration: Boolean(regenerate) });
    const action = regenerate ? (selection.action === "shorten" ? "shorten" :
    ...
    try {
      ...
      const response = await fetch("/api/ats-rewrite", {
        ...
        body: JSON.stringify({
          schemaVersion: 1, requestId, cvId, expectedContentVersion: getVersion(),
      ...
      if (!result.success || body?.schemaVersion !== 1 || body?.requestId !== requestId) throw new Error("SUGGESTION_FAILED");
      if (run !== sequence.current || abort.signal.aborted) return;
      if (currentKey(getData()) !== key) throw new Error("STALE_DOCUMENT");
      const incoming = writingChanges(source, result.data, target)
        .filter(change => !regenerate || change.field === regenerate.field)
      ...
      setChanges(old => regenerate && retainOtherChanges
        ...
        : incoming);
      track("ai_writing_result", { locale, target: analyticsTarget, changeCount: incoming.length });
      if (!incoming.length) setNotice(tr("Geen wijzigingen voorgesteld. Je tekst is behouden.", "No changes suggested. Your text has been kept."));
      ...
      track("ai_writing_failed", { locale, target: analyticsTarget, reason: code.slice(0, 48) || "unknown" });
  ...
  function accept(change: WritingChange) {
    ...
    try {
      ...
      setChanges(old => old.map(item => item.id === change.id ? { ...item, status: "accepted" } : item));
      ...
      track("ai_writing_decision", { locale, target: analyticsTarget, field: change.field, decision: "accepted" });
  ...
  function undo(item: HistoryItem) {
    ...
    try {
      ...
      track("ai_writing_decision", { locale, target: analyticsTarget, field: item.field, decision: "undone" });
    } catch {
  ...
  const label = (change: WritingChange) => change.field === "summary" ? tr("Profiel", "Profile") :
    change.field === "description" ? tr("Functiebeschrijving", "Job description") : tr("Bulletpunten", "Bullet points");
  ...
  return (
    <dialog ref={dialog} onCancel={onClose} aria-labelledby="ai-writing-title"
      ...
      {selection.target.kind !== "all" && <label className="my-4 block text-sm font-medium">
