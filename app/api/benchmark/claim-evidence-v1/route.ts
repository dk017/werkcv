...
export async function GET(request: NextRequest) {
  if (!claimEvidenceBenchmarkReleaseReady()) return NextResponse.json({ error: "The independently reviewed dataset has not been published yet.", code: "BENCHMARK_REVIEW_PENDING" }, { status: 404, headers: { "Cache-Control": "no-store" } });
