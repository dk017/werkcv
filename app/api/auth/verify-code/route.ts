...
function resolvePostLoginPath(nextPath: string | null, attribution: ReturnType<typeof sanitizeAttribution>) {
    ...
    const isEnglishIntent = sourcePath.startsWith('/en') || attribution?.locale === 'en';
    if (!isEnglishIntent) return nextPath || '/templates';
    if (!nextPath || nextPath === '/templates' || nextPath === '/editor') return '/en/templates';
    if (nextPath === '/prijzen') return '/en/templates';
    return nextPath;
}
...
export async function POST(request: NextRequest) {
    try {
                ...
                await prisma.analyticsEvent.create({
                    data: {
                        ...
                        properties: {
                            ...
                            sourceLocale: attribution?.firstTouchPath?.startsWith('/en') ? 'en' : attribution?.locale || null,
        ...
        const response = NextResponse.json({ ok: true, userId: session.userId, isNewUser: session.isNewUser, redirectTo });
