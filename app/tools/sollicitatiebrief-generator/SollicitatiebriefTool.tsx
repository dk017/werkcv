...
export default function SollicitatiebriefTool() {
    ...
    async function handleGenerate() {
        if (!doelrol.trim() || motivatie.trim().length < 20) {
            setError('Vul je doelrol en een korte motivatie in (minimaal 20 tekens).');
            return;
        }
        setError('');
        setIsLoading(true);
        ...
        try {
            const res = await fetch('/api/tools/sollicitatiebrief', {
                ...
                body: JSON.stringify({ naam, doelrol, bedrijfsnaam, motivatie, toon }),
            });
            ...
            {!result ? (
                <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-black uppercase tracking-wide text-slate-600 mb-1.5">
                                Doelrol <span className="text-red-500">*</span>
                            </label>
                            <input
                                value={doelrol}
                                onChange={e => setDoelrol(e.target.value)}
                                placeholder="bijv. UX Designer"
                                className={inputClass}
                            />
                        </div>
                    </div>
                    ...
                    <div>
                        <label className="block text-xs font-black uppercase tracking-wide text-slate-600 mb-1.5">
                            Jouw motivatie & achtergrond <span className="text-red-500">*</span>
                        </label>
                        <textarea
                            value={motivatie}
                            onChange={e => { setMotivatie(e.target.value); setError(''); }}
                            placeholder="bijv. 5 jaar ervaring als UX designer in e-commerce. Gespecialiseerd in mobile-first design en usability testing. Wil graag naar een product-gedreven bedrijf waar ik meer impact kan maken..."
                            rows={5}
                            className={`${inputClass} resize-none`}
                        />
                        <p className="text-xs text-slate-400 mt-1">Meer context = een sterkere, persoonlijkere brief.</p>
                    </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-black uppercase tracking-wide text-slate-500">Jouw sollicitatiebrief</span>
