...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const result = await getJobListingPageByPath(`/vacatures/${slug}`);

  if (!result || !result.meetsThresholds) {
    return { title: "Vacatures pagina niet gevonden | WerkCV.nl" };
  }
...
export default async function DutchJobsListingPage({ params }: PageProps) {
  const { slug } = await params;
  const result = await getJobListingPageByPath(`/vacatures/${slug}`);
