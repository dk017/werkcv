    ...
    <>
      <section className="mx-auto max-w-6xl px-5 py-12 sm:px-6 sm:py-16">
        <div className="grid gap-8 lg:grid-cols-[0.74fr_1.26fr] lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">{isEnglish ? "How to read the result" : "Zo leest u de uitslag"}</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">{isEnglish ? "A source line is a starting point, not a guarantee." : "Een bronregel is een startpunt, geen garantie."}</h2>
            <p className="mt-4 text-sm leading-relaxed text-slate-600">{isEnglish ? "The tool is intentionally conservative. It separates what the vacancy asks for, what the CV appears to say and what a recruiter still needs to verify. That makes the output useful for review without pretending that a text model can certify a candidate." : "De tool is bewust terughoudend. Hij scheidt wat de vacature vraagt, wat het CV lijkt te zeggen en wat een recruiter nog moet controleren. Zo is de uitslag bruikbaar voor review zonder te doen alsof een tekstmodel een kandidaat kan certificeren."}</p>
          </div>
          <div className="overflow-x-auto border-2 border-slate-950 bg-white">
            <table className="w-full min-w-[680px] border-collapse text-left text-sm">
              <thead className="bg-slate-950 text-white"><tr><th className="p-4">{isEnglish ? "Status" : "Status"}</th><th className="p-4">{isEnglish ? "What the tool found" : "Wat de tool vond"}</th><th className="p-4">{isEnglish ? "Recruiter action" : "Actie voor recruiter"}</th></tr></thead>
              <tbody>
                {(isEnglish ? [
      ...
      <section className="border-y-2 border-slate-950 bg-[#E9FFFC]">
        <div className="mx-auto max-w-6xl px-5 py-12 sm:px-6 sm:py-16">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-sky-700">{isEnglish ? "Method" : "Methode"}</p>
          <h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">{isEnglish ? "Four checks, with the limits shown." : "Vier controles, met de grenzen erbij."}</h2>
          <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            ]).map(([number, title, body]) => <article key={number} className="border-2 border-slate-950 bg-white p-5"><span className="font-mono text-sm font-black text-emerald-700">{number}</span><h3 className="mt-3 text-lg font-black">{title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-600">{body}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-12 sm:px-6 sm:py-16">
        <div className="grid gap-8 lg:grid-cols-2">
          <article className="border-2 border-rose-300 bg-rose-50 p-6"><p className="text-xs font-black uppercase tracking-[0.16em] text-rose-800">{isEnglish ? "Do not use it for" : "Niet gebruiken voor"}</p><h2 className="mt-3 text-2xl font-black">{isEnglish ? "Automated hiring decisions" : "Automatische aannamebeslissingen"}</h2><ul className="mt-5 space-y-3 text-sm leading-relaxed text-rose-950">{(isEnglish ? ["Ranking or rejecting candidates without human review", "Inferring protected characteristics or personal traits", "Turning a keyword into a claim of competence", "Replacing candidate consent, intake or reference checks"] : ["Kandidaten rangschikken of afwijzen zonder menselijke review", "Beschermde kenmerken of persoonlijke eigenschappen afleiden", "Een zoekwoord omzetten in een competentieclaim", "Toestemming, intake of referentiecontrole vervangen"]).map((item) => <li key={item} className="flex gap-3"><span className="font-black">—</span><span>{item}</span></li>)}</ul></article>
          <article className="border-2 border-emerald-400 bg-emerald-50 p-6"><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-800">{isEnglish ? "Use it for" : "Wel gebruiken voor"}</p><h2 className="mt-3 text-2xl font-black">{isEnglish ? "A consistent pre-send review" : "Een consistente controle vóór verzending"}</h2><ul className="mt-5 space-y-3 text-sm leading-relaxed text-emerald-950">{(isEnglish ? ["Finding which requirements still need a candidate question", "Giving a reviewer a source line to verify", "Keeping evidence and open points separate", "Deciding whether a full MatchPack review is worthwhile"] : ["Zien welke eisen nog een vraag aan de kandidaat nodig hebben", "Een reviewer een bronregel geven om te controleren", "Bewijs en open punten uit elkaar houden", "Bepalen of een volledige MatchPack-review zinvol is"]).map((item) => <li key={item} className="flex gap-3"><span className="font-black">✓</span><span>{item}</span></li>)}</ul></article>
        </div>
      </section>

      <section className="border-y-2 border-slate-950 bg-slate-950 text-white">
        <div className="mx-auto max-w-6xl px-5 py-12 sm:px-6 sm:py-16">
          <div className="grid gap-8 lg:grid-cols-[1fr_1fr]">
            <div><p className="text-xs font-black uppercase tracking-[0.18em] text-yellow-300">{isEnglish ? "Trust and data" : "Vertrouwen en data"}</p><h2 className="mt-3 text-3xl font-black tracking-tight">{isEnglish ? "A useful result must also be safe to use." : "Een bruikbare uitslag moet ook verantwoord te gebruiken zijn."}</h2><p className="mt-4 text-sm leading-relaxed text-slate-300">{isEnglish ? "Use fictional or authorised candidate data. Explain how an AI-assisted tool is used, keep a human in the loop and document the limits of the output. For real MatchPack processing, ask for the current DPA and subprocessor information before deployment." : "Gebruik fictieve of correct geautoriseerde kandidaatdata. Leg uit hoe een AI-ondersteunde tool wordt gebruikt, houd een mens in de controle en documenteer de grenzen van de uitslag. Vraag vóór gebruik met echte MatchPack-data de actuele DPA- en subverwerkersinformatie op."}</p><Link href="/agency/privacy#subprocessors" className="mt-5 inline-block text-sm font-black text-yellow-300 underline decoration-2 underline-offset-4">{isEnglish ? "Read WerkCV privacy and processor information →" : "Lees WerkCV privacy- en verwerkersinformatie →"}</Link></div>
            <div className="border-2 border-slate-700 bg-slate-900 p-5"><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">{isEnglish ? "External guidance" : "Externe bronnen"}</p><div className="mt-4 space-y-4 text-sm leading-relaxed text-slate-300"><a href="https://www.gov.uk/government/publications/responsible-ai-in-recruitment-guide/responsible-ai-in-recruitment" target="_blank" rel="noreferrer" className="block underline decoration-slate-500 underline-offset-4 hover:text-white">{isEnglish ? "UK Government: Responsible AI in Recruitment" : "UK Government: Responsible AI in Recruitment (Engels)"}</a><a href="https://ico.org.uk/about-the-ico/media-centre/news-and-blogs/2024/11/thinking-of-using-ai-to-assist-recruitment-our-key-data-protection-considerations" target="_blank" rel="noreferrer" className="block underline decoration-slate-500 underline-offset-4 hover:text-white">{isEnglish ? "ICO: data protection considerations for AI-assisted recruitment" : "ICO: aandachtspunten voor AI bij recruitment"}</a><a href="https://www.nvp-hrnetwerk.nl/l/library/download/urn%3Auuid%3Ac902b6f9-dcac-4c26-9a13-8b90f4d2fe5f/mos25068%2Bbrochure%2Bsollicitatiecode%2Bonline.pdf" target="_blank" rel="noreferrer" className="block underline decoration-slate-500 underline-offset-4 hover:text-white">{isEnglish ? "NVP Application Code 2025" : "NVP Sollicitatiecode 2025"}</a></div></div>
          </div>
        </div>
      </section>

      <section className="mx-auto max-w-6xl px-5 py-12 sm:px-6 sm:py-16"><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">FAQ</p><h2 className="mt-3 text-3xl font-black tracking-tight">{isEnglish ? "Questions agencies should answer before sharing a candidate." : "Vragen die bureaus vóór het delen van een kandidaat moeten beantwoorden."}</h2><div className="mt-6 space-y-3">{faqs.map((faq) => <details key={faq.question} className="border-2 border-slate-200 bg-white p-4 open:border-slate-950"><summary className="cursor-pointer font-black">{faq.question}</summary><p className="mt-3 max-w-4xl text-sm leading-relaxed text-slate-600">{faq.answer}</p></details>)}</div></section>
