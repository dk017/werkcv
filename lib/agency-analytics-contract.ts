...
// Internal paths are intentionally stricter than generic labels. Agency
// acquisition events must never receive query strings (which can contain
// identifiers) or external URLs. Anchors are allowed because the public
// content links use section destinations such as `/agency#plan`.
const internalPath = z
  .string()
  .trim()
  .min(1)
  .max(180)
  .regex(/^\/[^\s?#]*(?:#[A-Za-z0-9_-]+)?$/, "Expected an internal path without a query string");
const locale = z.enum(["nl", "en"]);
const outputVariant = z.enum(["full", "anonymized", "contact_reduced"]);
const agencyGuideLocale = z.literal("nl");
const agencyGuideDeviceCategory = z.enum(["mobile", "tablet", "desktop", "unknown"]);
const agencyGuideSourceCategory = z.enum(["search", "ai", "social", "email", "referral", "direct", "internal", "unknown"]);
...
export const agencyAnalyticsEventSchemas: Record<string, z.ZodTypeAny> = {
  agency_hub_viewed: z.object({ path: internalPath }).strict(),
  agency_guide_index_viewed: z.object({ path: internalPath }).strict(),
  agency_guide_viewed: z.object({ path: internalPath, slug: shortText }).strict(),
  agency_public_sector_guide_viewed: z.object({
    route_id: z.literal("nl_public_sector_submission"),
    locale: agencyGuideLocale,
    device_category: agencyGuideDeviceCategory,
    source_category: agencyGuideSourceCategory,
  }).strict(),
  agency_example_viewed: z.object({ path: internalPath, slug: shortText }).strict(),
  ...
  agency_pricing_viewed: z.object({ locale, path: internalPath }).strict(),
  ...
  agency_docx_cta_clicked: z.object({ path: internalPath, location: shortText }).strict(),
  agency_redaction_cta_clicked: z.object({ path: internalPath, location: shortText }).strict(),
  agency_evidence_matrix_downloaded: z.object({
