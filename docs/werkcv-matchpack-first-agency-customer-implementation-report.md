...
- `lib/agency-access.ts` — nullish-safe limits, actual allowance in errors and credit terminology.
...
Migration rehearsal status: **not run in this pass**. A production-compatible PostgreSQL backup, restore rehearsal, before/after aggregate counts and a second-run no-op still require the owner/environment gate. Do not report existing-customer treatment as complete until that audit is recorded without customer emails in the repository.
...
The full-use revenue arithmetic is implemented (`EUR 99 / 300 = EUR 0.33 per credit`) and is labelled as a full-use reference, not a billing promise. Financial certification is **pending** because paid provider/model rate cards, worst-path call counts, file limits, email/storage costs and payment fees must be recorded with source URLs and dates. Do not publish a margin or savings claim until that worksheet is completed.
...
| Command | Result |
|---|---|
| `npm run test:agency:acquisition` | PASS — 11 tests |
| `npm run test:agency:activation` | PASS — 29 tests |
...
| `npm run build` | PASS — 554 routes generated; existing dynamic-font status-400 warning only |
